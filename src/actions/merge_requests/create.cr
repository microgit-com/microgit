class Repositories::MergeRequests::Create < RepositoryAction
  post "/:namespace_slug/:repository_slug/merge_requests" do
    SaveMergeRequest.create(params, repository_id: @repository.not_nil!.id, author_id: current_user.id, status: MergeRequest::AvramStatus.new(:open)) do |operation, merge_request|
      if merge_request
